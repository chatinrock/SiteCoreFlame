...
$artListCount = count($artList);
for ($i = 0; $i < $artListCount; $i++) {
    $item = $artList[$i];
    ...
    <div class="content clearfix">
        <?
        $text = @file_get_contents($articleDir . $item['idSplit'] . 'kat.txt');
        if ($text) {
            echo $text;
        } else {
            self::loadFile($articleDir . $item['idSplit'] . 'data.txt');
        }
        ?>
    </div>
