...
var stationMap = (function () {
	...
	function setStationSelect(pStationIdList){
		for( var i in pStationIdList ){
			selectStation(pStationIdList[i]);
		} // for
		// func. setStationSelect
	}
    ...
    function _remove(pStationId) {
        ...
        // Удаляем контейнер
        jQuery(marketBox + ' .st' + pStationId).remove();
        return true;
        // func. _remove
    }
    ...
    function init(pOptions) {
        jQuery(pOptions.mapBox).click(mapClick);
        // func. init
    }
    ...
    return {
		...
		getStationSelect: getStationSelect,
...
function animFrame(pDivObj) {
    ...
    this.nextFrame = function (pOptions) {
        ...
        jQuery(divObj).css('background-position', '-' + xPos + 'px -' + yPos + 'px');
...
var animPng = (function () {
	...
	function clearAll(){
        ...
        // Останавливаем таймер
        clearInterval(intervalId);
		jQuery(options.markerBox+' .'+options.markerClass).remove();
    ...
    function addMarker(pStationId, pX, pY) {
        ...
        jQuery(markerBox).append(divObj);
...
var explorerMvc = (function(){
	...
	function selectBtnClick(){
		explorerMvc.onCallback(stationMap.getStationSelect());
		window.close();
		// func. selectBtnClick
	}
	...
	function initData(data){
		console.log(data);
		stationMap.setStationSelect(data.list);
		// func. initData
	}
	
	function init(){
		findObj = new findBtn("#findDrop", function() {
			if ( this.selected ){
		...
		jQuery('#findIcon').click(function(event) {
			findObj.select();
			findObj.click();
		});
		
		jQuery('#clearBtn').click(animPng.clearAll);
