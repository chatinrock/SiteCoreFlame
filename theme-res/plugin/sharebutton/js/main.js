var WPCOMSharing = {
	get_counts : function( url ) {
		if ( jQuery( '#sharing-facebook-' + WPCOM_sharing_counts[ url ] ).length ){
			...
			src += encodeURIComponent( "SELECT total_count, url FROM link_stat WHERE url='" + url + "'" );
			src += '&format=json&callback=WPCOMSharing.updateFacebookCount';
			console.log(src);
	...
	updateFacebookCount : function( data ) {
