var layerMvc = (function(){
	...
	function loadResDivImage(list){
		setTimeout(function(){
			for(var num in list ){
				var image = new Image();
				var curLayer = layerList[currentLayerId].list;
				console.log(curLayer[list[num]].src);
	...
	function addImages(data, pType){
		...
		for( var num in list ){
			jQuery( '#box-'+list[num] ).draggable({ containment: "#sliderShowBox", scroll: false, stop: dragStop });
	...
	function setLayerRes(pLayerData){
		...
		for( var num in listNew ){
			jQuery('#box-'+listNew[num] ).draggable({ containment: "#sliderShowBox", scroll: false, stop: dragStop });
		} // for
